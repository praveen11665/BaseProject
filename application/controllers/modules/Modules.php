...
class Modules extends MY_Controller 
{
  ...
  {
    //Page Config
    $viewData['dataTableUrl']  = 'modules/Modules/datatable';
    $viewData['ActionUrl']     = 'modules/Modules/formSubmit';
    $viewData['listData']      = '1';
    //redirect 
    ...
    $data = array(
                    'title'     =>  $this->dbvars->app_name.' - '.$this->lang->line('module_form_title'),
                    'content'   =>  $this->load->view('modules/modules_form', $viewData, TRUE)
                  );
  ...
  public function formSubmit($viewData='')
  {
    ...
    {
     //Validation Rules
      $this->form_validation->set_rules('category_code', 'Module', 'required|alpha');          
      $this->form_validation->set_rules('category_desc', 'Module Description', 'required|trim');      
      ...
      if($this->form_validation->run() == TRUE)
      {
        ...
        if($this->input->post('category_id') == "")
        {
          ...
          if($result)
          {
            ...
            redirect(base_url('modules/Modules/add'));
          }
        }
        else
        { 
  ...
  {
    $this->datatables ->select('c.category_code, c.category_desc, c.category_id')
    ->from('acl_categories as c');                                       
    $this->datatables ->edit_column('c.category_id', '$1', 'get_buttons(c.category_id, "modules/modules/")');
