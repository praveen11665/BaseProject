  ...
  class Datatables
  {
    ...
    */
    protected function get_paging()
    {
      ...
      $this->ci->db->limit(($iLength != '' && $iLength != '-1') ? $iLength : 100000, ($iStart)? $iStart : 0);
    }
        ...
        foreach($custom_val['replacement'] as $key => $val)
        {
          ...
          if(preg_match('/(\w+)\((.*)\)/i', $val, $matches) && function_exists($matches[1]))
          {
           ...
           // $replace_string = call_user_func_array($func, $args);
			...
			//var_dump($args);
			if($args[0] == "getStatusLable"){
			 $replace_string = $this->getStatusActive($args[1]);
			} 			
			
			if($args[0] == "getQualityAssure"){
			 $replace_string = $this->getStatus($args[1]);
			} 	
			
			if($args[0] == "getProductStatus"){
			 $replace_string = $this->getProductStatus($args[1]);
			} 	
    ...
    */
    protected function jsonify($result = FALSE)
    {
	 ...
	 function getStatusActive($id)
	 {
		return ($id==1) ? '<strong style="color:green;"> Active </strong>' : '<strong style="color:red;">In Active</strong>';
	 }
	 
	 function getStatus($id)
	 {
		return ($id==1) ? '<strong style="color:green;"> Yes </strong>' : '<strong style="color:red;">No</strong>';
	 }
